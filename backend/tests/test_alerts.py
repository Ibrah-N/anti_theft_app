...
def create_test_alerts(client, auth_headers):
    ...
    client.post("/api/vehicle/engine", json={"state": False}, headers=auth_headers)
    client.post("/api/vehicle/fuel",   json={"state": False}, headers=auth_headers)
...
def test_mark_all_alerts_read(client, auth_headers):
    response = client.patch("/api/alerts/read-all", headers=auth_headers)
    assert response.status_code == 200
